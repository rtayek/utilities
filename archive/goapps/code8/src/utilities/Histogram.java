...
public class Histogram {
    public static @SuppressWarnings("serial") class LruMap<K,V> extends LinkedHashMap<K,V> {
        public LruMap(int max) { super(max+1); this.max=max; }
        @Override public boolean removeEldestEntry(Map.Entry<K,V> eldest) { return size()>max; }
        final int max;
    }
    ...
    public double maxDifference() { // what exactly is this?
        ...
        double max=0;
        for(int i=0;i<bins();i++) max=Math.max(max,Math.abs(bin(i)-n()/(double)bins())/(n()/(double)bins()));
        return max;
    }
