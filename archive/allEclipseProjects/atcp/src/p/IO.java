...
// maybe fix this to use shutdown and is shutdown?
public class IO {
    static class Connection extends Thread {
        Connection(Socket socket,Consumer<String> stringConsumer,
                Consumer<Exception> exceptionConsumer,boolean outGoing) {
            this.socket=socket;
            this.stringConsumer=stringConsumer;
            this.exceptionConsumer=exceptionConsumer;
            setName("connection #"+serialNumber+(outGoing?" to: ":" from: ")
                    +socket);
        ...
        boolean send(String string) {
            try {
                ...
                if(isClosing);
                    ...
                    String o=other==null?"?":other.isClosing.toString();
                    p(this+" in send, not closing, other: "+o);
                    e.printStackTrace();
                }
                return false;
            }
        ...
        @Override public void run() {
            try {
                ...
                if(isClosing);
                    ...
                    String o=other==null?"?":other.isClosing.toString();
                    p(this+" in run, not closing, other: "+o);
                    e.printStackTrace();
                    unexpected++;
                ...
                try {
                    ...
                    try {
                        ...
                        if(this.getState().equals(Thread.State.TERMINATED)
                            ...
                            ;
                        else p("thread has strange state after join: "
                                +toS(this));
        ...
        public static Socket connect(InetSocketAddress inetSocketAddress,
                int timeout,Consumer<Exception> exceptionConsumer) {
            ...
            try {
                ...
                try {
                    ...
                    else p("in connect, close, caught: "+e2);
                }
                socket=null;
            }
            return socket;
    ...
    static class Acceptor extends Thread {
            ...
            synchronized(isClosing) {
                ...
                try {
                    ...
                    try {
                        ...
                        // p("thread: "+toS(this));
                        if(!this.getState().equals(Thread.State.TERMINATED))
                            p("2 thread has strange state after join: "
                                    +toS(this));
                    } catch(InterruptedException e) {
                        e.printStackTrace();
                    }
        ...
        static boolean bind(ServerSocket serverSocket,
                SocketAddress socketAddress) { // move to acceptor?
        ...
        static Acceptor acceptor(SocketAddress socketAddress,
    ...
    public static void p(String string) { p(System.out,string); }
    public static void pe(String string) { p(System.err,string); }
    public static String toS(Thread thread) {
        return "thread: name: "+thread.getName()+", state: "+thread.getState()
                +", is alive: "+thread.isAlive()+", is interrupted:  "
                +thread.isInterrupted();
    }
    public static String toS(ServerSocket serverSocket) {
        return serverSocket+": isBound: "+serverSocket.isBound()+", isClosed: "
                +serverSocket.isClosed();
    }
    public static String toS(InterfaceAddress interfaceAddress) {
        InetAddress a=interfaceAddress.getAddress();
        String s=""+interfaceAddress.getAddress()+" "
                +interfaceAddress.getNetworkPrefixLength();
        s+=" "+a.isSiteLocalAddress()+" "+a.isAnyLocalAddress()+" "
    ...
    public static void printThreads(List<String> excluded) {
        ...
        for(Thread thread:threads) if(thread!=null
                &&(excluded==null||!excluded.contains(thread.getName())))
            p(toS(thread));
    }
    ...
    public void run() throws IOException,InterruptedException {
        ...
        final Consumer<String> stringConsumer=new Consumer<String>() {
            @Override public void accept(String string) {
                ...
                if(!incoming.send("bar")) IO.p("send was not ok!");
            }
        };
        ...
        Consumer<Socket> consumer=new Consumer<Socket>() {
            @Override public void accept(Socket socket) {
                incoming=new IO.Connection(socket,stringConsumer,
                        exceptionConsumer,false);
                incoming.start(); // start listening
            }
        };
        ...
        // but copy of IO in rubiks project fails!
    ...
    // maybe move this to tst/ as an interation or functional test?
    ...
    static List<String> excluded=Arrays
            .asList(new String[] {"main","ReaderThread"});
